...
#print "debug: before mutate call, generate_population()\n";
  @pop = @{mutate(\@pop)};
...
sub run_stochhmm {
  ...
  for (my $n = 0; $n < @hmms; $n++) { ## WARNING!!! FIXME MODIFIED FOR CRICK
    push(@comm, "/home/arrajpur/bin/stochhmm/StochHMM-master/stochhmm -model $hmms[$n] -seq $seq -posterior -threshold 0.9 -gff > $dir$n.report");
...
#      $newarr[0]{emm} = @{crossover($pop[$father_iter]{emm}, $pop[$mother_iter]{emm})};
      $newarr[0]{emm} = crossover($pop[$father_iter]{emm}, $pop[$mother_iter]{emm});
#	print "debug mate(): before mutate dump:\n";
#	print Dumper(@newarr);
      @newarr = @{mutate(\@newarr)};
#	print "debug mate(): after mutate dump:\n";
#	print Dumper(@newarr);
...
#        push(@{$new[$state]}, @line1); #FIXME check this!
        push(@{$new[$state]},[@line1]); #FIXME check this! # changed
      }
      elsif ($cross_row == $row) {
        my @line1 = @{$father[$state][$row]};
        ...
        for (my $col = 0; $col < $NUM_EMISSIONS; $col++) {
          if ($row < $cross_col) {
#		print "line 1: $state $row $col $line1[$col]\n";
            $new[$state][$row][$col] = $line1[$col];
          }
          else {
#		my $debug = $line2[$col];
#		my @debug2 = $line2[$col];
#		print "line 2: $state $row $col $line2[$col] sizeof line2 " . @line2 . " line2[col] = $line2[$col], debug is $debug sizeof debug2 is " .@debug2 ."new[state][row][col] is [$new[$state][$row][$col]]\n";
...
#        push(@{$new[$state]}, @line2); #FIXME check this!
        push(@{$new[$state]}, [@line2]); #FIXME check this! # changed
      }
    }
  }
	
#print "Dumping new: New appears fine\n";
#print Dumper(@new);
  return(\@new);
}
